...
#
# Por defecto: Davidgelves/NoxuraSSH (mismo repo que el instalador actual).
...
SSHPLUS_GH_USER_REPO="$(_sshplus_trim "${SSHPLUS_GH_USER_REPO:-Davidgelves/NoxuraSSH}")"
[[ -z "$SSHPLUS_GH_USER_REPO" ]] && SSHPLUS_GH_USER_REPO="Davidgelves/NoxuraSSH"
...
if [[ "$_r_lc" == *usuario/tu-repo* ]] || [[ "$_r_lc" == "tu_usuario/tu_repo" ]] || [[ "$_r_lc" == "usuario/repo" ]]; then
	SSHPLUS_GH_USER_REPO="Davidgelves/NoxuraSSH"
fi
unset SSHPLUS_RAW
